use cosmwasm_std::{
    ...
    InitResponse, MessageInfo, MigrateResponse, Order, StdError, StdResult, Uint128,
};
...
pub fn handle(
    ...
    match msg {
        ...
        HandleMsg::RobinHood {} => handle_robin_hood(deps, env, info),
        HandleMsg::RedistibuteWealth {} => handle_redistribute_wealth(deps, env, info),
    }
}
...
pub fn handle_robin_hood(
    ...
    let mut richest_acc: Vec<u8> = vec![];
    let mut richest_amount: Uint128 = Uint128::zero();

    let mut accounts = balances(deps.storage);

    let accounts_array: StdResult<Vec<AccountInfo>> = accounts
        .range(None, None, Order::Ascending)
        .map(|item| item.and_then(|(k, v)| Ok(AccountInfo { addr: k, amount: v })))
        .collect();

    for acc in accounts_array? {
        if acc.amount > richest_amount {
            richest_amount = acc.amount;
            richest_acc = acc.addr;
        }
    }

    let robbed_amount = richest_amount.multiply_ratio(1u64, 2u64);

    accounts.update(
        sender_raw.as_slice(),
        |balance: Option<Uint128>| -> StdResult<_> {
            Ok(balance.unwrap_or_default() + robbed_amount)
        },
    )?;
    accounts.update(&richest_acc, |balance: Option<Uint128>| {
        balance.unwrap_or_default() - robbed_amount
    })?;

    let robbed_human_addr = deps.api.human_address(&CanonicalAddr::from(richest_acc))?;

    let res = HandleResponse {
        messages: vec![],
        attributes: vec![
            attr("action", "robin_hood"),
            attr("robber", deps.api.human_address(&sender_raw)?),
            attr("robbed", robbed_human_addr),
            attr("amount", richest_amount),
        ],
        data: None,
    };
    Ok(res)
...
pub fn handle_redistribute_wealth(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
) -> Result<HandleResponse, ContractError> {
    ...
    let mut number_of_addresses = 0u64;
    let mut total_amount: Uint128 = Uint128::zero();

    let mut accounts = balances(deps.storage);

    let addresses: StdResult<Vec<Vec<u8>>> = accounts
        .range(None, None, Order::Ascending)
        .map(|item| {
            item.and_then(|(k, v)| {
    ...
    let average: Uint128 = total_amount.multiply_ratio(1u64, number_of_addresses);

    for addr in addresses? {
        accounts.update(&addr, |_: Option<Uint128>| -> StdResult<_> { Ok(average) })?;
    }

    let res = HandleResponse {
        messages: vec![],
        attributes: vec![
            attr("action", "redistribute_wealth"),
            attr("sender", deps.api.human_address(&sender_raw)?),
            attr("wealthPerPerson", average),
        ],
        data: None,
    };
    Ok(res)
